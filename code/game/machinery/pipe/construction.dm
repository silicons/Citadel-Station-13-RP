...
/obj/item/pipe/proc/setPipingLayer(new_layer = PIPING_LAYER_DEFAULT)
	...
	if(initial(fakeA.pipe_flags) & (PIPE_STATIC_FLAG_ALL_LAYER|PIPE_STATIC_FLAG_DEFAULT_LAYER_ONLY))
...
/obj/item/pipe/wrench_act(obj/item/I, datum/event_args/actor/clickchain/e_args, flags, hint)
	...
	for(var/obj/machinery/atmospherics/M in loc)
		if((M.pipe_flags & initial_flags & PIPE_STATIC_FLAG_ONE_PER_TURF))	//Only one dense/requires density object per tile, eg connectors/cryo/heater/coolers.
			e_args.chat_feedback(SPAN_WARNING("Something is hogging the tile!"), src)
			return TRUE
		if((M.piping_layer != piping_layer) && !((M.pipe_flags | initial_flags) & PIPE_STATIC_FLAG_ALL_LAYER)) // Pipes on different layers can't block each other unless they are ALL_LAYER
			continue
