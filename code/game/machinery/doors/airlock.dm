...
/obj/machinery/door/airlock/ui_interact(mob/user, datum/tgui/ui, datum/tgui/parent_ui, datum/tgui_state/custom_state)
...
/obj/machinery/door/airlock/attackby(obj/item/C, mob/user as mob)
	//to_chat(world, "airlock attackby src [src] obj [C] mob [user]")
	if(!istype(usr, /mob/living/silicon))
		if(src.isElectrified())
