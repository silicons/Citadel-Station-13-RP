...
// porting in lines from main
// disabled for now, we'll need to move to these later
// only stuff that needs to be ported should be zone check (put using advanced do after since this is rare) and range checks (put on both)
