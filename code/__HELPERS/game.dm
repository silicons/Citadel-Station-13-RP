	...
	for(var/i = 1; i <= GLOB.player_list.len; i++)
		...
		if(M)
			...
			if(ear)
				...
				if(speaker_coverage[ear] || (istype(M, /mob/observer/dead) && M.get_preference_toggle(/datum/game_preference_entry/toggle/observer_ghost_radio)))
	...
	for(var/mob in GLOB.player_list)
		...
		if(M && M.stat == DEAD && remote_ghosts && !M.forbid_seeing_deadchat)
			switch(type)
				...
				if(1)
					if(M.get_preference_toggle(/datum/game_preference_entry/toggle/observer_ghost_ears))
						mobs |= M
				// Visual messages use ghost_sight.
				if(2)
					if(M.get_preference_toggle(/datum/game_preference_entry/toggle/observer_ghost_sight))
						mobs |= M
