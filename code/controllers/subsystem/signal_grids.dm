...
SUBSYSTEM_DEF(signal_grids)
	...
	var/datum/signal_grid/living
	/// /obj/vehicle
	var/datum/signal_grid/vehicles
	/// /obj/overmap/entity's
	var/datum/signal_grid/overmap_entities

/datum/controller/subsystem/signal_grids/Initialize()
	make_grids()
	return SS_INIT_SUCCESS

/datum/controller/subsystem/signal_grids/proc/make_grids()
	living = new /datum/signal_grid(/mob/living)
	vehicles = new /datum/signal_grid(/obj/vehicle)
	overmap_entities = new /datum/signal_grid(/obj/overmap/entity)

/datum/controller/subsystem/signal_grids/on_max_z_changed(old_z_count, new_z_count)
	. = ..()
	if(!initialized)
		return
...
/datum/signal_grid/New(expected_type, init_flags)
	// initialize grid
...
/datum/signal_grid/proc/sync_world_z(size)
 ...
 */
/datum/signal_grid/proc/direct_insert(atom/movable/AM, z, index)
 ...
 * removes a movable
 */
/datum/signal_grid/proc/direct_remove(atom/movable/AM, z, index)
	var/list/grid = grids[z]
	var/entry = grid[index]
	if(islist(entry))
		entry -= AM
 ...
 */
/datum/signal_grid/proc/range_query(turf/epicenter, distance)
	. = list()
 ...
 * @return list() of atoms
 */
/datum/signal_grid/proc/pixel_query(atom/epicenter, distance)
	. = list()
 ...
 */
/datum/signal_grid/proc/all_atoms(z)
	if(optimize_get_all_on_z)
		if(z)
...
/datum/signal_grid/proc/reservation_range_query(datum/map_reservation/reservation, turf/epicenter, distance)
...
/datum/signal_grid/proc/reservation_all_atoms(datum/map_reservation/reservation)
	. = list()
...
/datum/signal_grid/proc/automatic_range_query(turf/epicenter, distance)
