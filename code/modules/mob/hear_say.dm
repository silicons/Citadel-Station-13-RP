...
/mob/proc/hear_say(var/message, var/verb = "says", var/datum/prototype/language/language = null, var/alt_name = "",var/italics = 0, var/mob/speaker = null, var/sound/speech_sound, var/sound_vol)
	if(!client && !teleop)
		return

	if(speaker && !speaker.client && istype(src,/mob/observer/dead) && get_preference_toggle(/datum/game_preference_entry/toggle/observer_ghost_ears) && !(speaker in view(src)))
	...
	if(istype(src, /mob/observer/dead))
		...
		if(get_preference_toggle(/datum/game_preference_entry/toggle/observer_ghost_ears) && (speaker in view(src)))
			message = "<b>[message]</b>"
	...
	if(is_deaf())
		...
		if(language)
			...
			if(client && !(language.language_flags & LANGUAGE_HIVEMIND) && say_understands(speaker, language) && language.shorthand && client.get_preference_toggle(/datum/game_preference_entry/toggle/chat_show_language_indicators))
		...
		if(check_mentioned(message) && get_preference_toggle(/datum/game_preference_entry/toggle/chat_legacy_name_highlight))
...
/mob/living/hear_say(var/message, var/verb = "says", var/datum/prototype/language/language = null, var/alt_name = "",var/italics = 0, var/mob/speaker = null, var/sound/speech_sound, var/sound_vol)
	..()
...
/mob/proc/language_scramble(datum/prototype/language/L, str)
...
/mob/living/silicon/language_scramble(datum/prototype/language/L, str)
...
/mob/proc/hear_radio(var/message, var/verb="says", var/datum/prototype/language/language=null, var/part_a, var/part_b, var/part_c, var/mob/speaker = null, var/hard_to_hear = 0, var/vname ="")
...
/mob/proc/on_hear_radio(part_a, speaker_name, track, part_b, formatted)
	...
	if(check_mentioned(formatted) && get_preference_toggle(/datum/game_preference_entry/toggle/chat_legacy_name_highlight))
