	...
	if(compactor)
		...
		if(istype(target, /obj/item) || istype(target, /obj/effect/decal/remains))
			...
			if(do_after(user, 30, target) && length(contents) < max_item_count)
				...
				playsound(hound, gulpsound, vol = 60, vary = 1, falloff = 0.1, preference = /datum/game_preference_entry/toggle/vore_sound_eating_noises)
		...
		if(istype(target, /mob/living/simple_mob/animal/passive/mouse)) //Edible mice, dead or alive whatever. Mostly for carcass picking you cruel bastard :v
			...
			if(do_after(user, 30, trashman) && !patient && !trashman.buckled && length(contents) < max_item_count)
				...
				playsound(hound, gulpsound, vol = 100, vary = 1, falloff = 0.1, preference = /datum/game_preference_entry/toggle/vore_sound_eating_noises)
				if(delivery)
					if(islist(deliverylists[delivery_tag]))
...
/obj/item/dogborg/sleeper/proc/clean_cycle()
	...
	if(!length(touchable_items))
		...
		playsound(hound, finisher, vol = 100, vary = 1, falloff = 0.1, ignore_walls = TRUE, preference = /datum/game_preference_entry/toggle/vore_sound_digestion_noises)
		to_chat(hound, "<span class='notice'>Your [src.name] is now clean. Ending self-cleaning cycle.</span>")
		cleaning = 0
		update_patient()
		playsound(hound, 'sound/machines/ding.ogg', vol = 100, vary = 1, falloff = 0.1, ignore_walls = TRUE, preference = /datum/game_preference_entry/toggle/vore_sound_digestion_noises)
		return
	...
	if(prob(20))
		...
		playsound(hound, churnsound, vol = 100, vary = 1, falloff = 0.1, ignore_walls = TRUE, preference = /datum/game_preference_entry/toggle/vore_sound_digestion_noises)
			...
			if((T.status_flags & STATUS_GODMODE) || !T.digestable)
				...
				if(T.stat == DEAD)
					...
					var/deathsound = pick(
						...
						'sound/vore/death9.ogg',
						'sound/vore/death10.ogg')
					playsound(hound, deathsound, vol = 100, vary = 1, falloff = 0.1, ignore_walls = TRUE, preference = /datum/game_preference_entry/toggle/vore_sound_digestion_noises)
