...
/mob/living/simple_mob/animal/sif/frostfly
	...
	ai_holder_type = /datum/ai_holder/fsm/simple_mob/ranged/kiting/threatening/frostfly
...
/datum/ai_holder/fsm/simple_mob/ranged/kiting/threatening/frostfly
	can_flee = TRUE
	dying_threshold = 0.5
	flee_when_outmatched = TRUE
	run_if_this_close = 3

/datum/ai_holder/fsm/simple_mob/ranged/kiting/threatening/frostfly/special_flee_check()
	var/mob/living/simple_mob/animal/sif/frostfly/F = holder
	if(F.energy < F.max_energy * 0.2)
		return TRUE
	return FALSE

/datum/ai_holder/fsm/simple_mob/ranged/kiting/threatening/frostfly/pre_special_attack(atom/A)
	if(isliving(A))
		holder.a_intent = INTENT_DISARM
	else
		holder.a_intent = INTENT_HARM

/datum/ai_holder/fsm/simple_mob/ranged/kiting/threatening/frostfly/post_ranged_attack(atom/A)
	var/mob/living/simple_mob/animal/sif/frostfly/F = holder
