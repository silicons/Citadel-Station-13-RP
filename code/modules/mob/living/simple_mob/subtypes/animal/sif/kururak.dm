...
/mob/living/simple_mob/animal/sif/kururak
	...
	ai_holder_type = /datum/ai_holder/fsm/simple_mob/intentional/kururak
	...
	if(has_modifier_of_type(/datum/modifier/ace))
		for(var/mob/living/simple_mob/animal/sif/kururak/K in hearers(7, src))
			...
			if(K.faction != src.faction)
				continue
			var/datum/ai_holder/fsm/AI = K.ai_holder
...
/datum/ai_holder/fsm/simple_mob/intentional/kururak
	hostile = FALSE
	retaliate = TRUE
	cooperative = TRUE
	can_flee = TRUE
...
/datum/ai_holder/fsm/simple_mob/intentional/kururak/handle_special_strategical()
...
/datum/ai_holder/fsm/simple_mob/intentional/kururak/pre_special_attack(atom/A)
...
/datum/ai_holder/fsm/simple_mob/intentional/kururak/post_melee_attack()
	if(holder.has_modifier_of_type(/datum/modifier/ace))
		request_help()
