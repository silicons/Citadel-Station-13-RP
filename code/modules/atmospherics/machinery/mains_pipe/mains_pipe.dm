...
/obj/machinery/atmospherics/mains_pipe/split/atmos_init()
	...
	spawn(5)
		if(node3 && split_node)
			var/datum/pipenet/N1 = node3.return_network(src)
			var/datum/pipenet/N2 = split_node.return_network(split_node)
			if(N1 && N2)
				N1.merge(N2)
...
/obj/machinery/atmospherics/mains_pipe/split3/atmos_init()
	...
	spawn(5)
		if(supply_node)
			var/datum/pipenet/N1 = supply_node.return_network(src)
			var/datum/pipenet/N2 = supply.return_network(supply)
			if(N1 && N2)
				N1.merge(N2)
		if(scrubbers_node)
			var/datum/pipenet/N1 = scrubbers_node.return_network(src)
			var/datum/pipenet/N2 = scrubbers.return_network(scrubbers)
			if(N1 && N2)
				N1.merge(N2)
		if(aux_node)
			var/datum/pipenet/N1 = aux_node.return_network(src)
			var/datum/pipenet/N2 = aux.return_network(aux)
			if(N1 && N2)
				N1.merge(N2)
