...
/obj/item/pda/ui_data(mob/user, datum/tgui/ui, datum/tgui_state/state)
	var/list/data = ..()
...
/obj/item/pda/ui_act(action, list/params, datum/tgui/ui, datum/tgui_state/state)
	if(..())
		return TRUE
	...
	switch(action)
		...
		if("Ringtone")
			...
			if(current_app)
				. = current_app.ui_act(action, params, ui, state)
