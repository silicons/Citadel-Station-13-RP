...
/obj/item/nif/proc/unimplant(var/mob/living/carbon/human/H)
	for(var/i in 1 to length(nifsofts))
		...
		if(!NS.active)
			continue
		NS.deactivate(TRUE)
	var/datum/nifsoft/simulation_core/SC = imp_check(NIF_SOULCATCHER)
...
/obj/item/nif/legacy_mob_melee_hook(mob/target, mob/user, clickchain_flags, list/params, mult, target_zone, intent)
