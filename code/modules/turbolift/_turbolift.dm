 ...
 * Turbolifts reloaded
 *
 * Blackjack and hookers, CITRP edition.
 * Coded in a way to make transitioning to /datum/shuttle and /atom/docking_port easy
 * For now though, uses ChangeTurf and forceMove directly.
 */
/datum/turbolift
	/// Name
	var/name = "Unidentified Turbolift"

	// Instantiation
	/// LOWER LEFT tile we spawn/ed at
	...
	var/height = 4
	/// Entrance directions
	var/entrance_directions = EAST
	/// Entrance size
	var/entrance_size = 2
	/// Attempt to center entrance? If not, will cram in far as possible counterclockwise
	var/entrance_center = TRUE
	/// Floor type - path or list
	var/floor_type = LIFT_DEFAULT_WALL
	/// Area type - path or list
	var/area_type = LIFT_DEFAULT_AREA
	/// Wall type - path or list
	var/wall_type = LIFT_DEFAULT_WALL
	/// Door type - path or list
	var/door_type = LIFT_DEFAULT_DOOR
	...
	/// Current floor
	var/current_floor
	/// Called floors by datum
	var/list/calls
	/// Moving to priority floor - Interior panel will be locked out until we reach this, when entering priority call
	var/datum/floor/priority_move_to
	/// Priority call entered world.time
