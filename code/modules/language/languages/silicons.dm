...
/datum/prototype/language/binary/broadcast(var/mob/living/speaker,var/message,var/speaker_mask)
	...
	for (var/mob/M in dead_mob_list)
		if(!istype(M,/mob/new_player) && !istype(M,/mob/living/carbon/brain)) //No meta-evesdropping
			...
			if(M.check_mentioned(message) && M.get_preference_toggle(/datum/game_preference_entry/toggle/chat_legacy_name_highlight))
	...
	for (var/mob/living/S in living_mob_list)
		...
		if(S.check_mentioned(message) && S.get_preference_toggle(/datum/game_preference_entry/toggle/chat_legacy_name_highlight))
			message_to_send = "<font size='3'><b>[message_to_send]</b></font>"
