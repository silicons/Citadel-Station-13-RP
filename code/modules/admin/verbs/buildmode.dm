		...
		if(9) // AI control
			if(pa.Find("left"))
				if(isliving(object))
					...
					// Reset processes.
					if(pa.Find("ctrl"))
						if(!isnull(L.ai_polaris_get_stance())) // Null means there's no AI datum or it has one but is player controlled w/o autopilot on.
					...
					// Toggle hostility
					if(pa.Find("alt"))
						if(!isnull(L.ai_polaris_get_stance()))
							var/datum/ai_holder/polaris/AI = L.ai_holder
