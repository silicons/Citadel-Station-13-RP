...
/datum/ai_holder/polaris/proc/request_help()
	ai_log("request_help() : Entering.", AI_LOG_LEGACY_DEBUG)
	if(!cooperative || ((world.time - last_helpask_time) < 10 SECONDS))
		return

	ai_log("request_help() : Asking for help.", AI_LOG_LEGACY_INFO)
...
//	for(var/mob/living/L in range(call_distance, holder))
	for(var/mob/living/L in faction_friends)
		...
		if(holder.IIsAlly(L))
			...
			if(L.client && call_players) // Dealing with a player.
				ai_log("request_help() : Asking [L] (Player) for help.", AI_LOG_LEGACY_INFO)
				to_chat(L, "<span class='critical'>\The [holder] [called_player_message]</span>")

			else if(L.ai_holder) // Dealing with an AI.
				ai_log("request_help() : Asking [L] (AI) for help.", AI_LOG_LEGACY_INFO)
	...
	ai_log("request_help() : Exiting.", AI_LOG_LEGACY_DEBUG)

// What allies receive when someone else is calling for help.
/datum/ai_holder/polaris/proc/help_requested(mob/living/friend)
	ai_log("help_requested() : Entering.", AI_LOG_LEGACY_DEBUG)
	if(!friend.has_polaris_AI())
		return
	var/datum/ai_holder/polaris/friend_ai_holder = friend.ai_holder
	if(stance == STANCE_SLEEP)
		ai_log("help_requested() : Help requested by [friend] but we are asleep.", AI_LOG_LEGACY_INFO)
		return
	if(!cooperative)
		ai_log("help_requested() : Help requested by [friend] but we're not cooperative.", AI_LOG_LEGACY_INFO)
		return
	if(stance in STANCES_COMBAT)
		ai_log("help_requested() : Help requested by [friend] but we are busy fighting something else.", AI_LOG_LEGACY_INFO)
		return
	if(!can_act())
		ai_log("help_requested() : Help requested by [friend] but cannot act (stunned or dead).", AI_LOG_LEGACY_INFO)
		return
	if(!holder.IIsAlly(friend)) // Extra sanity.
		ai_log("help_requested() : Help requested by [friend] but we hate them.", AI_LOG_LEGACY_INFO)
		return
	if(friend_ai_holder && friend_ai_holder.target && !can_attack(friend_ai_holder.target))
		ai_log("help_requested() : Help requested by [friend] but we don't want to fight their target.", AI_LOG_LEGACY_INFO)
		return
	if(get_dist(holder, friend) <= follow_distance)
		ai_log("help_requested() : Help requested by [friend] but we're already here.", AI_LOG_LEGACY_INFO)
		return
	if(get_dist(holder, friend) <= vision_range) // Within our sight.
		ai_log("help_requested() : Help requested by [friend], and within target sharing range.", AI_LOG_LEGACY_INFO)
		if(friend_ai_holder) // AI calling for help.
			if(friend_ai_holder.target && can_attack(friend_ai_holder.target)) // Friend wants us to attack their target.
				...
				ai_log("help_requested() : Given target [target] by [friend]. Exiting", AI_LOG_LEGACY_DEBUG)
				return
	...
	ai_log("help_requested() : Help requested by [friend], going to go to friend.", AI_LOG_LEGACY_INFO)
	set_follow(friend, 10 SECONDS)
	ai_log("help_requested() : Exiting.", AI_LOG_LEGACY_DEBUG)
