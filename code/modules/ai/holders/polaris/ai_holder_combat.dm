...
/datum/ai_holder/polaris/proc/engage_target()
	ai_log("engage_target() : Entering.", AI_LOG_LEGACY_DEBUG)

	// Can we still see them?
//	if(!target || !can_attack(target) || (!(target in list_targets())) )
	if(QDELETED(target) || !can_attack(target))
		ai_log("engage_target() : Lost sight of target.", AI_LOG_LEGACY_TRACE)
		lose_target() // We lost them.

		if(!find_target()) // If we can't get a new one, then wait for a bit and then time out.
			set_stance(STANCE_IDLE)
			lost_target()
			ai_log("engage_target() : No more targets. Exiting.", AI_LOG_LEGACY_DEBUG)
			return
			// if(lose_target_time + lose_target_timeout < world.time)
			// 	ai_log("engage_target() : Unseen enemy timed out.", AI_LOG_LEGACY_TRACE)
			// 	set_stance(STANCE_IDLE) // It must've been the wind.
			// 	lost_target()
			// 	ai_log("engage_target() : Exiting.", AI_LOG_LEGACY_DEBUG)
			// 	return

			// // But maybe we do one last ditch effort.
			// if(!target_last_seen_turf || intelligence_level < AI_SMART)
			// 	ai_log("engage_target() : No last known position or is too dumb to fight unseen enemies.", AI_LOG_LEGACY_TRACE)
			// 	set_stance(STANCE_IDLE)
			// else
			// 	ai_log("engage_target() : Fighting unseen enemy.", AI_LOG_LEGACY_TRACE)
			// 	engage_unseen_enemy()
		else
			ai_log("engage_target() : Got new target ([target]).", AI_LOG_LEGACY_TRACE)

	var/distance = get_dist(holder, target)
	ai_log("engage_target() : Distance to target ([target]) is [distance].", AI_LOG_LEGACY_TRACE)
...
//	if(prob(special_attack_prob) && (distance >= special_attack_min_range) && (distance <= special_attack_max_range))
	if(holder.ICheckSpecialAttack(target))
		ai_log("engage_target() : Attempting a special attack.", AI_LOG_LEGACY_TRACE)
		on_engagement(target)
		if(special_attack(target)) // If this fails, then we try a regular melee/ranged attack.
			ai_log("engage_target() : Successful special attack. Exiting.", AI_LOG_LEGACY_DEBUG)
			return

	// Stab them.
	else if(distance <= 1 && !pointblank)
		ai_log("engage_target() : Attempting a melee attack.", AI_LOG_LEGACY_TRACE)
		on_engagement(target)
		melee_attack(target)
	...
	// Shoot them.
		...
		if(firing_lanes && !test_projectile_safety(target))
			...
			if(T)
				...
				holder.face_atom(target)
			ai_log("engage_target() : Could not safely fire at target. Exiting.", AI_LOG_LEGACY_DEBUG)
			return

		ai_log("engage_target() : Attempting a ranged attack.", AI_LOG_LEGACY_TRACE)
		ranged_attack(target)

	// Run after them.
	else if(!stand_ground)
		ai_log("engage_target() : Target ([target]) too far away. Exiting.", AI_LOG_LEGACY_DEBUG)
...
/datum/ai_holder/polaris/proc/test_projectile_safety(atom/movable/AM)
	ai_log("test_projectile_safety([AM]) : Entering.", AI_LOG_LEGACY_TRACE)

	// If they're right next to us then lets just say yes. check_trajectory() tends to spaz out otherwise.
	if(holder.Adjacent(AM))
		ai_log("test_projectile_check_safety() : Adjacent to target. Exiting with TRUE.", AI_LOG_LEGACY_TRACE)
		return TRUE
	...
	ai_log("test_projectile_check_safety() : Test projectile did[!would_hit_primary_target ? " NOT " : " "]hit \the [AM]", AI_LOG_LEGACY_DEBUG)

	// Make sure we don't have a chance to shoot our friends.
	for(var/a in hit_things)
		var/atom/A = a
		ai_log("test_projectile_check_safety() : Evaluating \the [A] ([A.type]).", AI_LOG_LEGACY_TRACE)
		if(isliving(A)) // Don't shoot at our friends, even if they're behind the target, as RNG can make them get hit.
			var/mob/living/L = A
			if(holder.IIsAlly(L))
				ai_log("test_projectile_check_safety() : Would threaten ally, exiting with FALSE.", AI_LOG_LEGACY_DEBUG)
				return FALSE
	...
	if(!would_hit_primary_target && !isturf(AM) && conserve_ammo)
		ai_log("test_projectile_check_safety() : conserve_ammo is set, and test projectile failed to hit primary target. Exiting with FALSE.", AI_LOG_LEGACY_DEBUG)
		return FALSE

	ai_log("test_projectile_check_safety() : Passed other tests, exiting with TRUE.", AI_LOG_LEGACY_TRACE)
	return TRUE
...
/datum/ai_holder/polaris/proc/walk_to_target()
	ai_log("walk_to_target() : Entering.", AI_LOG_LEGACY_DEBUG)
	// Make sure we can still chase/attack them.
	if(!target || !can_attack(target))
		ai_log("walk_to_target() : Lost target.", AI_LOG_LEGACY_INFO)
		if(!find_target())
			lost_target()
			ai_log("walk_to_target() : Exiting.", AI_LOG_LEGACY_DEBUG)
			return
		else
			ai_log("walk_to_target() : Found new target ([target]).", AI_LOG_LEGACY_INFO)

	// Find out where we're going.
	var/get_to = closest_distance(target)
	var/distance = get_dist(holder, target)
	ai_log("walk_to_target() : get_to is [get_to].", AI_LOG_LEGACY_TRACE)
	...
	if(distance <= get_to || holder.ICheckSpecialAttack(target))
		ai_log("walk_to_target() : Within range.", AI_LOG_LEGACY_INFO)
		forget_path()
...
/datum/ai_holder/polaris/proc/breakthrough(atom/target_atom)
	ai_log("breakthrough() : Entering", AI_LOG_LEGACY_TRACE)

	if(!can_breakthrough)
		ai_log("breakthrough() : Not allowed to breakthrough. Exiting.", AI_LOG_LEGACY_TRACE)
		return FALSE

	if(!isturf(holder.loc))
		ai_log("breakthrough() : Trapped inside \the [holder.loc]. Exiting.", AI_LOG_LEGACY_TRACE)
		return FALSE

	var/dir_to_target = get_dir(holder, target_atom)
	holder.face_atom(target_atom)
	ai_log("breakthrough() : Exiting", AI_LOG_LEGACY_DEBUG)
	...
	ai_log("breakthrough() : Starting peaceful pass.", AI_LOG_LEGACY_DEBUG)
	...
	if(!result && can_violently_breakthrough())
		ai_log("breakthrough() : Starting violent pass.", AI_LOG_LEGACY_DEBUG)
		for(var/direction in directions_to_try)
			result = destroy_surroundings(direction, violent = TRUE)
			if(result)
				break

	ai_log("breakthrough() : Exiting with [result].", AI_LOG_LEGACY_TRACE)
	return result
...
/datum/ai_holder/polaris/proc/destroy_surroundings(direction, violent = TRUE)
	ai_log("destroy_surroundings() : Entering.", AI_LOG_LEGACY_TRACE)
	if(!direction)
		direction = pick(GLOB.cardinal) // FLAIL WILDLY
		ai_log("destroy_surroundings() : No direction given, picked [direction] randomly.", AI_LOG_LEGACY_DEBUG)

	var/turf/problem_turf = get_step(holder, direction)

	// First, give peace a chance.
	if(!violent)
		ai_log("destroy_surroundings() : Going to try to peacefully clear [problem_turf].", AI_LOG_LEGACY_DEBUG)
		for(var/obj/machinery/door/D in problem_turf)
			if(D.density && holder.Adjacent(D) && D.allowed(holder) && D.operable())
				...
				ai_log("destroy_surroundings() : Opening closed door.", AI_LOG_LEGACY_INFO)
				return D.open()

	// Peace has failed us, can we just smash the things in the way?
	else
		ai_log("destroy_surroundings() : Going to try to violently clear [problem_turf].", AI_LOG_LEGACY_DEBUG)
		...
		for(var/obj/structure/window/W in problem_turf)
			if(W.dir == global.reverse_dir[holder.dir]) // So that windows get smashed in the right order
				ai_log("destroy_surroundings() : Attacking side window.", AI_LOG_LEGACY_INFO)
				return holder.IAttack(W)

			else if(W.fulltile)
				ai_log("destroy_surroundings() : Attacking full tile window.", AI_LOG_LEGACY_INFO)
				return holder.IAttack(W)
		...
		for(var/obj/effect/energy_field/shield in problem_turf)
			if(shield.density) // Don't attack shields that are already down.
				ai_log("destroy_surroundings() : Attacking hull shield.", AI_LOG_LEGACY_INFO)
		...
		for(var/obj/effect/shield/S in problem_turf)
			if(S.density) // Don't attack shields that are already down.
				ai_log("destroy_surroundings() : Attacking energy shield.", AI_LOG_LEGACY_INFO)
		...
		if(istype(obstacle, /obj/structure/window) || istype(obstacle, /obj/structure/closet) || istype(obstacle, /obj/structure/table) || istype(obstacle, /obj/structure/grille))
			ai_log("destroy_surroundings() : Attacking generic structure.", AI_LOG_LEGACY_INFO)
		...
		for(var/obj/machinery/door/D in problem_turf) // Required since firelocks take up the same turf.
			if(D.density)
				ai_log("destroy_surroundings() : Attacking closed door.", AI_LOG_LEGACY_INFO)
				return holder.IAttack(D)

	ai_log("destroy_surroundings() : Exiting due to nothing to attack.", AI_LOG_LEGACY_INFO)
