...
/datum/ai_holder/polaris/proc/forget_path()
	ai_log("forget_path() : Entering.", AI_LOG_LEGACY_DEBUG)
	if(path_display)
		for(var/turf/T in path)
			T.cut_overlay(path_overlay)
	path.Cut()
	ai_log("forget_path() : Exiting.", AI_LOG_LEGACY_DEBUG)

/datum/ai_holder/polaris/proc/give_up_movement()
	ai_log("give_up_movement() : Entering.", AI_LOG_LEGACY_DEBUG)
	forget_path()
	destination = null
	ai_log("give_up_movement() : Exiting.", AI_LOG_LEGACY_DEBUG)

/datum/ai_holder/polaris/proc/calculate_path(atom/A, get_to = 1)
	ai_log("calculate_path([A],[get_to]) : Entering.", AI_LOG_LEGACY_DEBUG)
	if(!A)
		ai_log("calculate_path() : Called without an atom. Exiting.",AI_LOG_LEGACY_WARNING)
		return

	if(!use_astar) // If we don't use A* then this is pointless.
		ai_log("calculate_path() : Not using A*, Exiting.", AI_LOG_LEGACY_DEBUG)
		return

	get_path(get_turf(A), get_to)

	ai_log("calculate_path() : Exiting.", AI_LOG_LEGACY_DEBUG)
...
/datum/ai_holder/polaris/proc/get_path(var/turf/target,var/get_to = 1, var/max_distance = world.view*6)
	ai_log("get_path() : Entering.",AI_LOG_LEGACY_DEBUG)
	forget_path()
	var/list/new_path = SSpathfinder.default_ai_pathfinding(src, target, get_to)

	if(new_path && new_path.len)
		path = new_path
		ai_log("get_path() : Made new path.",AI_LOG_LEGACY_DEBUG)
		if(path_display)
			for(var/turf/T in path)
				T.add_overlay(path_overlay)
	else
		ai_log("get_path() : Failed to make new path. Exiting.",AI_LOG_LEGACY_DEBUG)
		return 0

	ai_log("get_path() : Exiting.", AI_LOG_LEGACY_DEBUG)
	return path.len
