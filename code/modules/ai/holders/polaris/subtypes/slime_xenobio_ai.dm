...
/datum/ai_holder/polaris/simple_mob/xenobio_slime
	hostile = TRUE
	cooperative = TRUE
	firing_lanes = TRUE
...
/datum/ai_holder/polaris/simple_mob/xenobio_slime/Destroy()
	grudges.Cut()
	..()
...
/datum/ai_holder/polaris/simple_mob/xenobio_slime/passive/New() // For Kendrick.
	..()
	pacify()

/datum/ai_holder/polaris/simple_mob/xenobio_slime/New()
	..()
...
/datum/ai_holder/polaris/simple_mob/xenobio_slime/proc/is_justified_to_discipline()
	ai_log("xenobio_slime/is_justified_to_discipline() : Entered.", AI_LOG_LEGACY_TRACE)
	if(!can_act())
		ai_log("xenobio_slime/is_justified_to_discipline() : Judged to be unjustified because we cannot act. Exiting.", AI_LOG_LEGACY_DEBUG)
		return FALSE // The slime considers it abuse if they get stunned while already stunned.
	if(rabid)
		ai_log("xenobio_slime/is_justified_to_discipline() : Judged to be justified because we're rabid. Exiting.", AI_LOG_LEGACY_TRACE)
		return TRUE
	if(target && can_attack(target))
		if(ishuman(target))
			var/mob/living/carbon/human/H = target
			if(istype(H.species, /datum/species/monkey))
				ai_log("xenobio_slime/is_justified_to_discipline() : Judged to be unjustified because we're targeting a monkey. Exiting.", AI_LOG_LEGACY_DEBUG)
				return FALSE // Attacking monkeys is okay.
		ai_log("xenobio_slime/is_justified_to_discipline() : Judged to be justified because we are targeting a non-monkey. Exiting.", AI_LOG_LEGACY_TRACE)
		return TRUE // Otherwise attacking other things is bad.
	ai_log("xenobio_slime/is_justified_to_discipline() : Judged to be unjustified because we are not targeting anything. Exiting.", AI_LOG_LEGACY_DEBUG)
...
/datum/ai_holder/polaris/simple_mob/xenobio_slime/proc/adjust_discipline(amount, silent)
...
/datum/ai_holder/polaris/simple_mob/xenobio_slime/proc/discipline_decay()
...
/datum/ai_holder/polaris/simple_mob/xenobio_slime/handle_special_tactic()
	evolve_and_reproduce()

// Hit the correct verbs to keep the slime species going.
/datum/ai_holder/polaris/simple_mob/xenobio_slime/proc/evolve_and_reproduce()
	var/mob/living/simple_mob/slime/xenobio/my_slime = holder
...
/datum/ai_holder/polaris/simple_mob/xenobio_slime/proc/enrage()
	var/mob/living/simple_mob/slime/xenobio/my_slime = holder
...
/datum/ai_holder/polaris/simple_mob/xenobio_slime/proc/relax()
	...
	if(rabid)
		...
		my_slime.update_mood()
		lost_target()
		my_slime.visible_message(SPAN_NOTICE("\The [my_slime] calms down."))

// Called when using a pacification agent (or it's Kendrick being initalized).
/datum/ai_holder/polaris/simple_mob/xenobio_slime/proc/pacify()
...
/datum/ai_holder/polaris/simple_mob/xenobio_slime/pre_melee_attack(atom/A)
...
/datum/ai_holder/polaris/simple_mob/xenobio_slime/closest_distance(atom/movable/AM)
...
/datum/ai_holder/polaris/simple_mob/xenobio_slime/can_attack(atom/movable/AM)
	. = ..()
...
/datum/ai_holder/polaris/simple_mob/xenobio_slime/on_hear_say(mob/living/speaker, message)
	ai_log("xenobio_slime/on_hear_say([speaker], [message]) : Entered.", AI_LOG_LEGACY_DEBUG)
	var/mob/living/simple_mob/slime/xenobio/my_slime = holder
	...
	//		mass_order = TRUE
		...
		*/
	ai_log("xenobio_slime/on_hear_say() : Exited.", AI_LOG_LEGACY_DEBUG)
