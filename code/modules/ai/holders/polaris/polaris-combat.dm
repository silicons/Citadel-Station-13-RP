...
//	if(!target || !can_attack(target) || (!(target in list_targets())) )
	if(QDELETED(target) || !can_attack(target))
		polaris_ai_log("engage_target() : Lost sight of target.", POLARIS_AI_LOG_TRACE)
		lose_target() // We lost them.

		if(!find_target()) // If we can't get a new one, then wait for a bit and then time out.
			set_stance(STANCE_IDLE)
			lost_target()
			polaris_ai_log("engage_target() : No more targets. Exiting.", POLARIS_AI_LOG_DEBUG)
			return
			// if(lose_target_time + lose_target_timeout < world.time)
			// 	polaris_ai_log("engage_target() : Unseen enemy timed out.", POLARIS_AI_LOG_TRACE)
			// 	set_stance(STANCE_IDLE) // It must've been the wind.
			// 	lost_target()
			// 	polaris_ai_log("engage_target() : Exiting.", POLARIS_AI_LOG_DEBUG)
			// 	return

			// // But maybe we do one last ditch effort.
			// if(!target_last_seen_turf || intelligence_level < POLARIS_AI_SMART)
			// 	polaris_ai_log("engage_target() : No last known position or is too dumb to fight unseen enemies.", POLARIS_AI_LOG_TRACE)
			// 	set_stance(STANCE_IDLE)
			// else
			// 	polaris_ai_log("engage_target() : Fighting unseen enemy.", POLARIS_AI_LOG_TRACE)
			// 	engage_unseen_enemy()
		else
			polaris_ai_log("engage_target() : Got new target ([target]).", POLARIS_AI_LOG_TRACE)

	var/distance = get_dist(holder, target)
	polaris_ai_log("engage_target() : Distance to target ([target]) is [distance].", POLARIS_AI_LOG_TRACE)
...
//	if(prob(special_attack_prob) && (distance >= special_attack_min_range) && (distance <= special_attack_max_range))
	if(holder.ai_polaris_check_special_attack(target))
		...
		if(special_attack(target)) // If this fails, then we try a regular melee/ranged attack.
			polaris_ai_log("engage_target() : Successful special attack. Exiting.", POLARIS_AI_LOG_DEBUG)
			return
	...
	// Shoot them.
		...
		polaris_ai_log("engage_target() : Attempting a ranged attack.", POLARIS_AI_LOG_TRACE)
		ranged_attack(target)

	// Run after them.
	else if(!stand_ground)
		polaris_ai_log("engage_target() : Target ([target]) too far away. Exiting.", POLARIS_AI_LOG_DEBUG)
...
/datum/ai_holder/polaris/proc/test_projectile_safety(atom/movable/AM)
	polaris_ai_log("test_projectile_safety([AM]) : Entering.", POLARIS_AI_LOG_TRACE)

	// If they're right next to us then lets just say yes. check_trajectory() tends to spaz out otherwise.
	if(holder.Adjacent(AM))
		polaris_ai_log("test_projectile_check_safety() : Adjacent to target. Exiting with TRUE.", POLARIS_AI_LOG_TRACE)
		return TRUE
	...
	polaris_ai_log("test_projectile_check_safety() : Test projectile did[!would_hit_primary_target ? " NOT " : " "]hit \the [AM]", POLARIS_AI_LOG_DEBUG)
	...
	if(!would_hit_primary_target && !isturf(AM) && conserve_ammo)
		polaris_ai_log("test_projectile_check_safety() : conserve_ammo is set, and test projectile failed to hit primary target. Exiting with FALSE.", POLARIS_AI_LOG_DEBUG)
		return FALSE

	polaris_ai_log("test_projectile_check_safety() : Passed other tests, exiting with TRUE.", POLARIS_AI_LOG_TRACE)
	return TRUE
...
/datum/ai_holder/polaris/proc/walk_to_target()
	polaris_ai_log("walk_to_target() : Entering.", POLARIS_AI_LOG_DEBUG)
	// Make sure we can still chase/attack them.
	if(!target || !can_attack(target))
		polaris_ai_log("walk_to_target() : Lost target.", POLARIS_AI_LOG_INFO)
		if(!find_target())
			lost_target()
			polaris_ai_log("walk_to_target() : Exiting.", POLARIS_AI_LOG_DEBUG)
			return
		else
			polaris_ai_log("walk_to_target() : Found new target ([target]).", POLARIS_AI_LOG_INFO)

	// Find out where we're going.
	var/get_to = closest_distance(target)
	var/distance = get_dist(holder, target)
	polaris_ai_log("walk_to_target() : get_to is [get_to].", POLARIS_AI_LOG_TRACE)
...
/datum/ai_holder/polaris/proc/breakthrough(atom/target_atom)
	polaris_ai_log("breakthrough() : Entering", POLARIS_AI_LOG_TRACE)

	if(!can_breakthrough)
		polaris_ai_log("breakthrough() : Not allowed to breakthrough. Exiting.", POLARIS_AI_LOG_TRACE)
		return FALSE

	if(!isturf(holder.loc))
		polaris_ai_log("breakthrough() : Trapped inside \the [holder.loc]. Exiting.", POLARIS_AI_LOG_TRACE)
		return FALSE

	var/dir_to_target = get_dir(holder, target_atom)
	holder.face_atom(target_atom)
	polaris_ai_log("breakthrough() : Exiting", POLARIS_AI_LOG_DEBUG)
	...
	polaris_ai_log("breakthrough() : Starting peaceful pass.", POLARIS_AI_LOG_DEBUG)
	...
	if(!result && can_violently_breakthrough())
		polaris_ai_log("breakthrough() : Starting violent pass.", POLARIS_AI_LOG_DEBUG)
		for(var/direction in directions_to_try)
			result = destroy_surroundings(direction, violent = TRUE)
			if(result)
				break

	polaris_ai_log("breakthrough() : Exiting with [result].", POLARIS_AI_LOG_TRACE)
	return result
...
/datum/ai_holder/polaris/proc/destroy_surroundings(direction, violent = TRUE)
	polaris_ai_log("destroy_surroundings() : Entering.", POLARIS_AI_LOG_TRACE)
	if(!direction)
		direction = pick(GLOB.cardinal) // FLAIL WILDLY
		polaris_ai_log("destroy_surroundings() : No direction given, picked [direction] randomly.", POLARIS_AI_LOG_DEBUG)

	var/turf/problem_turf = get_step(holder, direction)

	// First, give peace a chance.
	if(!violent)
		polaris_ai_log("destroy_surroundings() : Going to try to peacefully clear [problem_turf].", POLARIS_AI_LOG_DEBUG)
		for(var/obj/machinery/door/D in problem_turf)
			if(D.density && holder.Adjacent(D) && D.allowed(holder) && D.operable())
				...
				polaris_ai_log("destroy_surroundings() : Opening closed door.", POLARIS_AI_LOG_INFO)
				return D.open()

	// Peace has failed us, can we just smash the things in the way?
	else
		polaris_ai_log("destroy_surroundings() : Going to try to violently clear [problem_turf].", POLARIS_AI_LOG_DEBUG)
		...
		for(var/obj/effect/shield/S in problem_turf)
			if(S.density) // Don't attack shields that are already down.
				polaris_ai_log("destroy_surroundings() : Attacking energy shield.", POLARIS_AI_LOG_INFO)
