...
/obj/item/vehicle_module/weapon/ranged/legacy/ballistic/mortar/action_checks(atom/target)
	var/turf/MT = get_turf(chassis)
	var/turf/TT = get_turf(target)
	if(!MT.outdoors || !TT.outdoors)
		to_chat(chassis.occupant_legacy, "<span class='notice'>\The [src]'s control system prevents you from firing due to a blocked firing arc.</span>")
		return 0
	return ..()
