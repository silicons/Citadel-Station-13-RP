...
/obj/belly/Entered(var/atom/movable/thing,var/atom/OldLoc)
	...
	if(vore_sound && !recent_sound)
		...
		if(soundfile)
			playsound(src, soundfile, vol = 100, vary = 1, falloff = VORE_SOUND_FALLOFF, preference = /datum/game_preference_entry/toggle/vore_sound_eating_noises)
			recent_sound = TRUE
...
/obj/belly/proc/handle_resist(var/mob/living/R)
	...
	if(is_wet)
		...
		playsound(src, struggle_snuggle, vary = 1, vol = 75, falloff = VORE_SOUND_FALLOFF, preference = /datum/game_preference_entry/toggle/vore_sound_digestion_noises)
	else
		playsound(src, struggle_rustle, vary = 1, vol = 75, falloff = VORE_SOUND_FALLOFF, preference = /datum/game_preference_entry/toggle/vore_sound_digestion_noises)
...
/obj/belly/proc/transfer_contents(var/atom/movable/content, var/obj/belly/target, silent = 0)
	...
	if(!silent && target.vore_sound && !recent_sound)
		...
		if(soundfile)
			playsound(src, soundfile, vol = 100, vary = 1, falloff = VORE_SOUND_FALLOFF, preference = /datum/game_preference_entry/toggle/vore_sound_digestion_noises)
	owner.updateVRPanel()
	for(var/mob/living/M in contents)
		M.updateVRPanel()
