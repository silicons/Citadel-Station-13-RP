		...
		if(isbelly(M.loc)) //sanity check
			if(world.time > M.next_preyloop) //We don't want it to overlap, but we also want it to replay.
				if(is_wet && wet_loop) // Is it a fleshy environment, and does the pred have a fleshy heartbeat loop to play?
					if(!M.client)
						continue
					if(M.get_preference_toggle(/datum/game_preference_entry/toggle/vore_sound_digestion_noises)) //then we check if the mob has sounds enabled at all
						...
						if(I)
							...
							if(item_digest_mode == IM_HOLD)
								...
								if(istype(I,/obj/item/reagent_containers/food) || istype(I,/obj/item/organ))
									...
									items_preserved |= I
								if(prob(25)) //Less often than with normal digestion
									if(L && L.client && L.get_preference_toggle(/datum/game_preference_entry/toggle/vore_sound_digestion_noises))
...
//////////////////////////// DM_DIGEST ////////////////////////////
		...
		if(prob(50)) //Was SO OFTEN. AAAA.
			for(var/mob/M in contents)
				if(M && M.client && M.get_preference_toggle(/datum/game_preference_entry/toggle/vore_sound_digestion_noises))
...
/////////////////////////// Make any noise ///////////////////////////
	if(play_sound)
		...
		for(var/mob/M in hearers(VORE_SOUND_RANGE, owner))
			if(!M.client || !(M.get_preference_toggle(/datum/game_preference_entry/toggle/vore_sound_digestion_noises)))
				continue
