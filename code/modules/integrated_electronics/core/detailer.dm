...
/obj/item/integrated_electronics/detailer/ui_data(mob/user, datum/tgui/ui, datum/tgui_state/state)
	var/list/data = ..()
	data["detail_color"] = detail_color
	data["color_list"] = color_list
	return data

/obj/item/integrated_electronics/detailer/ui_act(action, list/params, datum/tgui/ui, datum/tgui_state/state)
	if(..())
		return TRUE
